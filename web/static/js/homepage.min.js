...
var _map_utils = require('../map_utils');

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

var map = (0, _map_utils.getDefaultMap)({ zoom: 6.5 });
map.setTarget('map');
...
$(document).ready(function () {
    ...
    $.ajax({
        ...
        success: function success(result) {
            ...
            for (var i = 0; i < rapid_inv_geojson.features.length; i++) {
                rapid_inv_geojson.features[i].properties.type = "Inventaire rapide";
            }
            nb_layer_loaded += 1;
    ...
    $.ajax({
        ...
        success: function success(result) {
            ...
            for (var i = 0; i < taxa_inv_geojson.features.length; i++) {
                taxa_inv_geojson.features[i].properties.type = "Inventaire taxonomique";
            }
            nb_layer_loaded += 1;
            if (nb_layer_loaded == 2) {
                addInventoriesLayer(rapid_inv_geojson, taxa_inv_geojson);
    ...
    function addInventoriesLayer(rapid_inv_geojson, taxa_inv_geojson) {
        var fill = 'rgba(226, 188, 21, 0.7)';
        var stroke = '#474747';

        var features = new _openlayers2.default.Collection();
        var source = new _openlayers2.default.source.Vector({ features: features });
        var vector = new _openlayers2.default.layer.Vector({
            source: source,
        ...
        source.addFeatures(new _openlayers2.default.format.GeoJSON().readFeatures(rapid_inv_geojson));
        source.addFeatures(new _openlayers2.default.format.GeoJSON().readFeatures(taxa_inv_geojson));

        map.addLayer(vector);
        var hover_interaction = new _openlayers2.default.interaction.Select({
            condition: _openlayers2.default.events.condition.pointerMove,
            layers: [vector],
    ...
    function addPopupOverlay(layer) {
        var popup = new _map_utils.Popup({
        ...
        var point_select = new _openlayers2.default.interaction.Select({
        ...
        point_select.on('select', function (evt) {
            ...
            if (selection.getLength() > 0) {
                ...
                var content = point.get('type') + ' - ' + point.get('inventory_date') + ' - ' + point.get('location_description');
                popup.show(coord, title, content);
            } else {
                popup.hide();
            }
        });
        ...
        var bar = new ProgressBar.SemiCircle(container, {
            ...
            step: function step(state, circle) {
                ...
                if (value === 0) {
                    ...
                    circle.setText('<i class="fa ' + icon + '" aria-hidden="true"></i> ' + value + "<p>" + suffix + "</p>");
                }
            }
        });
    ...
    createProgressBar('#occurrence_count_container', niamoto_status.nb_occurrences, "#b3de69", " occurrences", "fa-pagelines");
    createProgressBar('#plot_count_container', niamoto_status.nb_plots, "#80b1d3", " parcelles", "fa-map-marker");
    createProgressBar('#provider_count_container', niamoto_status.nb_data_providers, "#969696", " sources", "fa-database");
    createProgressBar('#taxa_count_container', niamoto_status.nb_taxa, "#fb8072", " taxons", "fa-leaf");
});

}).call(this,typeof global !== "undefined" ? global : typeof self !== "undefined" ? self : typeof window !== "undefined" ? window : {})
},{"../map_utils":2}],2:[function(require,module,exports){
(function (global){
'use strict';
...
exports.getDefaultMap = getDefaultMap;
exports.getPolygonArea = getPolygonArea;
exports.makeGeoJSONLayer = makeGeoJSONLayer;
exports.Popup = Popup;
...
function getDefaultMap(options) {
    /**
    ...
    var view = new _openlayers2.default.View({
        projection: 'EPSG:4326',
        center: new _openlayers2.default.proj.transform([165.875, -21.145], 'EPSG:4326', 'EPSG:4326'),
        zoom: zoom
    });
    var attribution = new _openlayers2.default.control.Attribution({
        collapsible: false
    });
        ...
        layers: [new _openlayers2.default.layer.Tile({
            source: new _openlayers2.default.source.TileWMS({
                ...
                attributions: [new _openlayers2.default.Attribution({
                    html: '<a href="http://georep.nc/">Géorep</a> ' + '- <em>Gouvernement de la ' + 'Nouvelle-Calédonie</em>'
                })]
            })
        })],
        view: view,
        controls: [new _openlayers2.default.control.Zoom(), attribution]
    });
    return map;
};
...
function makeGeoJSONLayer(geojson, opt_options) {
    ...
    var source = new _openlayers2.default.source.Vector({
        features: new _openlayers2.default.format.GeoJSON().readFeatures(geojson)
    });
    options['source'] = source;
    if (image_vector) {
        return new _openlayers2.default.layer.Image({
            source: new _openlayers2.default.source.ImageVector(options)
        });
    } else {
        return new _openlayers2.default.layer.Vector(options);
    }
};
...
function Popup(opt_options) {
    ...
    _openlayers2.default.Overlay.call(this, {
...
_openlayers2.default.inherits(Popup, _openlayers2.default.Overlay);
 ...
 */
Popup.prototype.show = function (coord, title, content, callback) {
