...
from apps.niamoto_data.models import Occurrence, Plot
...
@transaction.atomic
def set_occurrences_rainfall(occurrences_ids=None):
    pass
    # occ_id_col = OccurrenceObservations.occurrence.field.get_attname()
    # if occurrences_ids is None:
    #     sql = \
    ...
    #             WITH rainfall AS (
    #                 SELECT occ.id AS id,
    #                     ST_Value(rainfall.rast, occ.location) AS rainfall
    #                 FROM {occ_table} AS occ
    #                 LEFT JOIN {rainfall_raster_table} AS rainfall
    #                 ON ST_Intersects(rainfall.rast, occ.location)
    #             )
    #             UPDATE {occ_obs}
    #             SET rainfall = rainfall.rainfall
    #             FROM rainfall
    #             WHERE rainfall.id = {occ_obs}.{occ_id_col};
    #         """.format(**{
    #             'occ_table': Occurrence._meta.db_table,
    #             'rainfall_raster_table': 'rainfall_wgs84',
    #             'occ_obs': OccurrenceObservations._meta.db_table,
    #             'occ_id_col': occ_id_col
    #         })
    # elif len(occurrences_ids) == 0:
